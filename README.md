...
Приложение будет доступно по адресу `localhost:8000`, или на порте, указанном в переменной `APP_PORT` в файле [`.env`](.env).
...
> Перед запуском убедитесь, что данные для подключения к базе данных в файле [`.env`](.env) соответствую вашей локально запущенной базе данных!
