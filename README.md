...
Для конфигурации приложения используется файл [`.env`](.env), для удобства запуска и проверки он уже расположен в репозитории и сконфигурирован для запуска с помощью `Docker`.
...
Приложение будет доступно по адресу `http://localhost:8000`, или на порте, указанном в переменной `APP_PORT` в файле [`.env`](.env).
...
> [!WARNING]
> Программа расчитана на использование `PostgreSQL`, если хотите воспользоваться другой СУБД, установите соответствующие зависимости и измените движок в файле [`src/settings.py`](src/settings.py).  
> Перед запуском убедитесь, что данные для подключения к базе данных в файле [`.env`](.env) соответствуют вашей локально запущенной базе данных!
...
После запуска приложения, документация OpenAPI будет доступна на эндпоинте `/docs`. Если вы запустили приложение на порте 8000, перейдите по [ссылке](http://localhost:8000/docs).
